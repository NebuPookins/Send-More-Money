...
object SendMoreMoney {
   ...
   */
  case class AddChecked(payload: PotentialMatch) extends SMMMessage
   ...
   */
  case class UsageChecked(payload: PotentialMatch) extends SMMMessage
  ...
  class Master(appConf: Config) extends Actor with ActorLogging {
    ...
    def receive = {
      ...
      case AddChecked(PotentialMatch(word1, word2, wordtotal, Some(solutionsFound), None)) =>
        /*
        ...
        checkShutdown()
      case UsageChecked(PotentialMatch(word1, word2, wordtotal, Some(addProofs), Some(usageProof: String))) =>
        /**
    ...
    def receive = {
      case PotentialMatch(operand1, operand2, total, _, maybeFreq) =>
        ...
        if (requirements.forall(predicate => predicate(operand1, operand2, total))) {
          ...
          sender ! AddChecked(PotentialMatch(operand1, operand2, total, Some(addProofs), maybeFreq))
        } else {
          sender ! AddChecked(PotentialMatch(operand1, operand2, total, Some(Set.empty), maybeFreq))
        }
    }
  }
      ...
      case PotentialMatch(word1, word2, word3, maybeAddProofs, None) =>
        ...
        val payload: PotentialMatch = 
          if (exampleUsage.isDefined) {
            ...
            PotentialMatch(word1, word2, word3, maybeAddProofs, None)
        }
        sender ! UsageChecked(payload)
    }
  }
}